...
var RestApiError = class _RestApiError extends AmplifyError {
  constructor(params) {
    super(params);
...
var buildRestApiServiceError = (error) => {
...
function createCancellableOperation(handler, abortController) {
  const isInternalPost = (handler2) => !!abortController;
...
// node_modules/@aws-amplify/api-rest/dist/esm/internals/index.mjs
var post4 = (input) => {
  return post3(Amplify, input);
};
  ...
  subscribe(options, customUserAgentDetails) {
    ...
    return new Observable((observer) => {
      if (!options || !appSyncGraphqlEndpoint) {
        ...
        reconnectSubscription = new Observable((observer2) => {
          this.reconnectionMonitor.addObserver(observer2);
        }).subscribe(() => {
          startSubscription();
        });
        startSubscription();
    ...
    if (subscriptionObserver) {
      ...
      if (subscriptionState === SUBSCRIPTION_STATUS.PENDING) {
        return new Promise((res, rej) => {
          const { observer, subscriptionState: subscriptionState2, variables, query } = subscriptionObserver;
          this.subscriptionObserverMap.set(subscriptionId, {
            observer,
...
// node_modules/@aws-amplify/api-graphql/dist/esm/utils/errors/APIError.mjs
...
var InternalGraphQLAPIClass = class {
  ...
  async _headerBasedAuth(amplify, authMode, additionalHeaders = {}) {
    var _a;
    const { region, endpoint: appSyncGraphqlEndpoint, apiKey } = resolveConfig(amplify);
    let headers = {};
    switch (authMode) {
      case "apiKey":
        if (!apiKey) {
  ...
  _graphqlSubscribe(amplify, { query, variables, authMode }, additionalHeaders = {}, customUserAgentDetails, authToken) {
    ...
    return this.appSyncRealTime.subscribe({
      ...
      authenticationType: authMode || (config == null ? void 0 : config.defaultAuthMode),
...
// node_modules/@aws-amplify/api-graphql/dist/esm/utils/resolveOwnerFields.mjs
...
// node_modules/@aws-amplify/api-graphql/dist/esm/internals/APIClient.mjs
var connectionType = {
  HAS_ONE: "HAS_ONE",
  HAS_MANY: "HAS_MANY",
  BELONGS_TO: "BELONGS_TO"
};
var flattenItems = (obj) => {
  const res = {};
...
var graphQLDocumentsCache = /* @__PURE__ */ new Map();
var SELECTION_SET_WILDCARD = "*";
function defaultSelectionSetForModel(modelDefinition) {
  const { fields: fields7 } = modelDefinition;
  const explicitFields = Object.values(fields7).map(({ type, name }) => (typeof type === "string" || typeof type === "object" && typeof (type == null ? void 0 : type.enum) === "string") && name).filter(Boolean);
...
function customSelectionSetToIR(modelDefinitions, modelName, selectionSet) {
  const dotNotationToObject = (path, modelName2) => {
    var _a, _b, _c;
    ...
    if (rest.length === 0) {
      ...
      const nested = rest[0];
      const modelDefinition = modelDefinitions[modelName2];
      const modelFields = modelDefinition.fields;
      const relatedModel = (_b = (_a = modelFields[fieldName]) == null ? void 0 : _a.type) == null ? void 0 : _b.model;
      if (!relatedModel) {
      ...
      if (nested === SELECTION_SET_WILDCARD) {
        ...
        if (!exists) {
          throw Error(`${fieldName} is not a field of model ${modelName2}`);
        }
        result = {
          [fieldName]: dotNotationToObject(rest.join("."), relatedModel)
        };
      }
      if ((_c = modelFields[fieldName]) == null ? void 0 : _c.isArray) {
        result = {
          [fieldName]: {
...
var defaultSelectionSetIR = (relatedModelDefinition) => {
...
function generateGraphQLDocument(modelDefinitions, modelName, modelOperation, listArgs) {
  var _a, _b;
  const modelDefinition = modelDefinitions[modelName];
  const { name, pluralName, fields: fields7, primaryKeyInfo: { isCustomPrimaryKey, primaryKeyFieldName, sortKeyFieldNames } } = modelDefinition;
  const { operationPrefix, usePlural } = graphQLOperationsInfo[modelOperation];
  const { selectionSet } = listArgs || {};
  const fromCache = (_a = graphQLDocumentsCache.get(name)) == null ? void 0 : _a.get(modelOperation);
  if (fromCache !== void 0) {
    return fromCache;
  }
  if (!graphQLDocumentsCache.has(name)) {
    graphQLDocumentsCache.set(name, /* @__PURE__ */ new Map());
  }
  ...
  switch (modelOperation) {
    ...
    case "READ":
      graphQLArguments ?? (graphQLArguments = isCustomPrimaryKey ? [primaryKeyFieldName, ...sortKeyFieldNames].reduce((acc, fieldName) => {
        acc[fieldName] = fields7[fieldName].type;
        return acc;
      }, {}) : {
  ...
  (_b = graphQLDocumentsCache.get(name)) == null ? void 0 : _b.set(modelOperation, graphQLDocument);
...
// node_modules/@aws-amplify/api-graphql/dist/esm/internals/operations/list.mjs
...
async function _list(client, modelIntrospection, model, args, contextSpec) {
  ...
  try {
    ...
    const headers = getCustomHeaders(client, args == null ? void 0 : args.headers);
...
// node_modules/@aws-amplify/api-graphql/dist/esm/internals/operations/get.mjs
...
async function _get(client, modelIntrospection, model, arg, options, operation, context) {
  ...
  try {
    ...
    const headers = getCustomHeaders(client, options == null ? void 0 : options.headers);
...
// node_modules/@aws-amplify/api-graphql/dist/esm/internals/operations/subscription.mjs
...
// node_modules/@aws-amplify/api-graphql/dist/esm/utils/resolvePKFields.mjs
...
// node_modules/@aws-amplify/api-graphql/dist/esm/utils/findIndexByFields.mjs
...
// node_modules/@aws-amplify/api-graphql/dist/esm/internals/operations/observeQuery.mjs
...
// node_modules/@aws-amplify/api-graphql/dist/esm/internals/generateModelsProperty.mjs
